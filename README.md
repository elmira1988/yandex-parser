...
### Раздел: Потенциальные улучшения системы

1. **Перевод парсинга в асинхронный фоновый режим (Laravel Queues + Cron)**
    * **Проблема:** Текущий запуск Headless Chromium внутри HTTP-запроса занимает много времени и может приводить к ошибкам таймаута веб-сервера (504 Gateway Time-out).
    * **Решение:** Вынос логики парсинга в изолированные фоновые задачи (Jobs) с использованием очередей (драйвер `database`) и контролем воркеров через системный планировщик `Cron` (`php artisan schedule:run`). Контроллер будет мгновенно возвращать ответ, а фронтенд — динамически опрашивать статус готовности данных.

2. **Оптимизация Chromium под ограничения инфраструктуры**
    * Переход на облегченную сборку браузера в Docker-контейнере (дистрибутив Alpine Linux, пакет `chromium` с флагами `--no-sandbox` и `--disable-dev-shm-usage`). Это позволит снизить потребление оперативной памяти на 60% и уменьшить дисковый след установки (отказ от тяжелых аудио/видео библиотек).
